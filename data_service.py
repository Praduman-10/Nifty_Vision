...
def _fetch(unit, interval, start, end):
    ...
    r=requests.get(url,headers={'Accept':'application/json','Authorization':f'Bearer {TOKEN}'},timeout=30); r.raise_for_status()
...
def get_market_data(label, limit=500):
    unit,interval=TIMEFRAMES[label]; key=f'{unit}_{interval}'
    cached=load_candles(KEY,key,limit)
    latest=latest_timestamp(KEY,key)
    try:
        start=pd.Timestamp(latest).date() if latest else date.today()-timedelta(days=7)
        fresh=_fetch(unit,interval,start,date.today())
