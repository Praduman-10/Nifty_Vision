...
def _fetch_intraday(unit, interval):
    # Historical V3 does not reliably expose the still-forming current candle.
    # Upstox's V3 intraday endpoint is specifically for the current trading day.
...
def get_market_data(label, limit=500):
    ...
    try:
        if label=='1 day':
            fresh=_fetch(unit,interval,end-timedelta(days=365),end)
        else:
            # Keep the historical cache current, then explicitly overlay today's
            # intraday feed so the active candle changes on every Streamlit rerun.
            recent_start=end-timedelta(days=2)
            if latest:
                latest_date=pd.Timestamp(latest).date()
                recent_start=min(latest_date,recent_start)
            historical=_fetch(unit,interval,recent_start,end)
