...
[data-testid="stSidebar"]{background:#080a0c;border-right:1px solid #20252b}[data-testid="stSidebar"]>div{padding-top:2rem}
.kicker{font-size:.62rem;font-weight:900;letter-spacing:2.4px;color:#6f7882;text-transform:uppercase}.title{font-size:2.7rem;font-weight:950;letter-spacing:-2.5px;line-height:1;margin:.35rem 0 .5rem}.sub{color:#727b86;font-size:.72rem}
.status,.pill{display:inline-block;border:1px solid #30363d;border-radius:999px;padding:4px 9px;font-size:.57rem;font-weight:850;letter-spacing:.8px;color:#aeb6bf}
.card{background:linear-gradient(145deg,#101316,#0a0c0e);border:1px solid #252b31;border-radius:14px;padding:15px 16px;min-height:92px;box-shadow:0 8px 24px rgba(0,0,0,.16)}.card:hover{border-color:#38414a}
.lab{font-size:.57rem;color:#737d87;font-weight:900;letter-spacing:1.4px;text-transform:uppercase}.val{font-size:1.35rem;font-weight:950;margin-top:7px;letter-spacing:-.5px}.green{color:#00e676}.red{color:#ff5252}.amber{color:#ffc107}.card-note{font-size:.62rem;color:#69727c;margin-top:4px}
.structure-card{background:linear-gradient(135deg,#0b1b12,#0a0d0c);border:1px solid #00e676;border-left:4px solid #00e676;border-radius:16px;padding:18px;box-shadow:0 0 30px rgba(0,230,118,.10);margin-bottom:12px}.structure-card.bear{background:linear-gradient(135deg,#1b0c0c,#0d0a0a);border-color:#ff5252;border-left-color:#ff5252;box-shadow:0 0 30px rgba(255,82,82,.10)}.structure-card.neutral{background:linear-gradient(135deg,#171507,#0d0c09);border-color:#ffc107;border-left-color:#ffc107}.structure-title{font-size:.6rem;font-weight:900;letter-spacing:1.8px;color:#9aa3ad}.structure-main{font-size:1.7rem;font-weight:950;margin:7px 0 2px}.structure-event{font-size:.75rem;color:#e2e6ea}.structure-conf{font-size:.67rem;color:#8b949e;margin-top:5px}
.panel{background:linear-gradient(145deg,#0d1012,#090b0d);border:1px solid #252b31;border-radius:16px;padding:17px;margin-bottom:12px;box-shadow:0 8px 25px rgba(0,0,0,.13)}.panel-head{display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid #1d2328;padding-bottom:11px;margin-bottom:13px}.pt{font-size:.76rem;font-weight:1000;letter-spacing:1.8px;text-transform:uppercase;color:#ffffff}.pattern-panel{padding:15px;background:linear-gradient(145deg,#0d1012,#080a0c);border:1px solid #252b31;border-radius:16px;box-shadow:0 8px 25px rgba(0,0,0,.13)}.pattern-panel-head{display:flex;justify-content:space-between;align-items:center;padding:2px 1px 12px;border-bottom:1px solid #252b31;margin-bottom:12px}.pattern-heading{font-size:.88rem;font-weight:1000;letter-spacing:2.2px;color:#fff;text-transform:uppercase}.pattern-card{padding:10px 11px;border:1px solid #20262c;border-radius:10px;background:#0a0c0e;margin-bottom:8px}.pattern-card.bull{border-left:3px solid #00e676}.pattern-card.bear{border-left:3px solid #ff5252}.pattern-card.neutral{border-left:3px solid #ffc107}.pattern-name{font-size:.72rem;font-weight:900;letter-spacing:.4px}.pattern-meta{font-size:.58rem;color:#77818b;margin-top:3px}.pattern-meaning{font-size:.67rem;color:#aab2ba;margin-top:5px;line-height:1.4}.divider{height:1px;background:#20252a;margin:18px 0}.read{font-size:.72rem;color:#999;line-height:1.5}
...
def make_chart(d,ps,events,frame,show_ema,show_vwap,show_sr,show_structure):
 ...
 (s1,s2),(r1,r2)=zones(d)
 if show_sr:
  f.add_hrect(y0=s1,y1=s2,fillcolor='rgba(0,230,118,.10)',line_color='#00e676',annotation_text='SUPPORT');f.add_hrect(y0=r1,y1=r2,fillcolor='rgba(255,82,82,.10)',line_color='#ff5252',annotation_text='RESISTANCE')
 if show_structure:
  for e in events:
   idx=e['index'];
...
n=st.sidebar.select_slider('CANDLES',options=[10,50,100,150,200,300,400,500,600,700,1000],value=300);st.sidebar.caption(f'{max_candles:,} candles loaded • showing {n:,}')
...
with right:
 card_cls='structure-card' if bias=='BULLISH' else 'structure-card bear' if bias=='BEARISH' else 'structure-card neutral';main_cls='green' if bias=='BULLISH' else 'red' if bias=='BEARISH' else 'amber';st.markdown(f"<div class='{card_cls}'><div class='structure-title'>MARKET STRUCTURE</div><div class='structure-main {main_cls}'>{struct['trend']}</div><div class='structure-event'>Latest: <b>{struct['latest']}</b></div><div class='structure-conf'>Confidence: <b>{struct['confidence']}</b></div></div>",unsafe_allow_html=True)
 pattern_html='<div class="pattern-panel"><div class="pattern-panel-head"><div class="pattern-heading">PATTERNS</div><div class="pill">LATEST</div></div>'
 for p in ps:
  cls='bull' if p['direction']=='bullish' else 'bear' if p['direction']=='bearish' else 'neutral';txt='BULLISH' if p['direction']=='bullish' else 'BEARISH' if p['direction']=='bearish' else 'NEUTRAL';color='green' if cls=='bull' else 'red' if cls=='bear' else 'amber';pattern_html+=f"<div class='pattern-card {cls}'><div class='pattern-name {color}'>{p['name']}</div><div class='pattern-meta'>{txt} • {p['confidence']} confidence • {p['score']}/3 confirmation</div><div class='pattern-meaning'>{p['meaning']}</div></div>"
 if not ps:pattern_html+='<div class="read">No recognised patterns on screen.</div>'
 pattern_html+='</div>'
