...
def indicators(d,frame):
 ...
 d['ema9']=d.close.ewm(span=9,adjust=False,min_periods=1).mean();d['ema20']=d.close.ewm(span=20,adjust=False,min_periods=1).mean();d['ema50']=d.close.ewm(span=50,adjust=False,min_periods=1).mean();d['session']=d['ts'].dt.date;tp=(d.high+d.low+d.close)/3;vol=d.volume.fillna(0)
