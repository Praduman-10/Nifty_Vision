...
uk_ts=last.ts.tz_convert(ZoneInfo('Europe/London'))
st.markdown('<div class="kicker">NIFTY 50 • PRICE ACTION TERMINAL</div><div class="title">Nifty Vision</div>',unsafe_allow_html=True);st.markdown(f'<div class="sub">{frame} • {uk_ts.strftime("%d %b %Y %H:%M:%S %Z")} • <span class="status">{source}</span> • <span class="status">LIVE • 30s</span></div>',unsafe_allow_html=True);st.markdown('<div class="divider"></div>',unsafe_allow_html=True)
cols=st.columns(7)
