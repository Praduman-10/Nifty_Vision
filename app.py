...
st.markdown('''<style>.stApp{background:#050505;color:#f5f5f5}.block-container{max-width:1800px;padding-top:2.5rem!important}[data-testid="stSidebar"]{background:#090909;border-right:1px solid #252525}.k{font-size:.65rem;font-weight:800;letter-spacing:2px;color:#777}.title{font-size:2.5rem;font-weight:950;letter-spacing:-2px}.sub{color:#707070;font-size:.75rem}.card{background:#0d0d0d;border:1px solid #292929;border-radius:14px;padding:14px}.lab{font-size:.6rem;color:#777;font-weight:800;letter-spacing:1px}.val{font-size:1.3rem;font-weight:900;margin-top:5px}.green{color:#00e676}.red{color:#ff5252}.amber{color:#ffc107}.panel{background:#0b0b0b;border:1px solid #252525;border-radius:15px;padding:16px}.structure-card{background:linear-gradient(135deg,#0b1710,#0b0b0b);border:1px solid #00e676;border-left:4px solid #00e676;border-radius:15px;padding:18px;box-shadow:0 0 24px rgba(0,230,118,.10)}.structure-card.bear{background:linear-gradient(135deg,#190b0b,#0b0b0b);border-color:#ff5252;border-left-color:#ff5252;box-shadow:0 0 24px rgba(255,82,82,.10)}.structure-card.neutral{background:#10100b;border-color:#ffc107;border-left-color:#ffc107}.structure-title{font-size:.65rem;font-weight:900;letter-spacing:1.5px;color:#aaa}.structure-main{font-size:1.65rem;font-weight:950;margin:7px 0 2px}.structure-event{font-size:.75rem;color:#ddd}.structure-conf{font-size:.68rem;color:#888;margin-top:5px}.pattern-card{background:#0b0b0b;border:1px solid #292929;border-radius:12px;padding:11px 12px;margin:8px 0}.pattern-card.bull{border-left:3px solid #00e676}.pattern-card.bear{border-left:3px solid #ff5252}.pattern-card.neutral{border-left:3px solid #ffc107}.pattern-name{font-size:.72rem;font-weight:900;letter-spacing:.5px}.pattern-meta{font-size:.62rem;color:#888;margin-top:3px}.pattern-meaning{font-size:.7rem;color:#aaa;margin-top:4px}.pt{font-size:.72rem;font-weight:900;letter-spacing:1.2px;text-transform:uppercase;margin-bottom:10px}.read{font-size:.76rem;color:#999;line-height:1.55}.status{display:inline-block;border:1px solid #303030;border-radius:999px;padding:5px 10px;font-size:.62rem;font-weight:800;letter-spacing:.7px;color:#aaa}</style>''',unsafe_allow_html=True)
...
with right:
 card_cls='structure-card' if bias=='BULLISH' else 'structure-card bear' if bias=='BEARISH' else 'structure-card neutral';main_cls='green' if bias=='BULLISH' else 'red' if bias=='BEARISH' else 'amber'
 st.markdown(f"<div class='{card_cls}'><div class='structure-title'>MARKET STRUCTURE</div><div class='structure-main {main_cls}'>{struct['trend']}</div><div class='structure-event'>Latest: <b>{struct['latest']}</b></div><div class='structure-conf'>Confidence: <b>{struct['confidence']}</b></div></div>",unsafe_allow_html=True)
 st.markdown("<br><div class='panel'><div class='pt'>PATTERNS</div>",unsafe_allow_html=True)
 for p in ps:
  cls='bull' if p['direction']=='bullish' else 'bear' if p['direction']=='bearish' else 'neutral';txt='BULLISH' if p['direction']=='bullish' else 'BEARISH' if p['direction']=='bearish' else 'NEUTRAL';st.markdown(f"<div class='pattern-card {cls}'><div class='pattern-name {('green' if cls=='bull' else 'red' if cls=='bear' else 'amber')}'>{p['name']}</div><div class='pattern-meta'>{txt} • {p['confidence']} confidence • {p['score']}/3 confirmation</div><div class='pattern-meaning'>{p['meaning']}</div></div>",unsafe_allow_html=True)
