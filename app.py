...
def indicators(d,frame):
 d=d.copy();d['ema9']=d.close.ewm(span=9,adjust=False,min_periods=1).mean();d['ema20']=d.close.ewm(span=20,adjust=False,min_periods=1).mean();d['ema50']=d.close.ewm(span=50,adjust=False,min_periods=1).mean()
 d['session']=d.ts.dt.date;tp=(d.high+d.low+d.close)/3;vol=pd.to_numeric(d.volume,errors='coerce').fillna(0)
 if frame=='1 day':
  # Daily VWAP is anchored over the loaded daily history; session VWAP on a 1D bar is just its typical price.
  cv=vol.cumsum();d['vwap']=(tp*vol).cumsum().div(cv.replace(0,np.nan))
 else:
  vv=vol.groupby(d.session).cumsum();d['vwap']=(tp*vol).groupby(d.session).cumsum().div(vv.replace(0,np.nan))
...
def make_chart(d,ps,frame,show_ema,show_vwap):
 ...
 if frame=='1 day':buttons=[dict(count=1,label='1M',step='month',stepmode='backward'),dict(count=3,label='3M',step='month',stepmode='backward'),dict(count=6,label='6M',step='month',stepmode='backward'),dict(count=1,label='1Y',step='year',stepmode='backward'),dict(step='all',label='ALL')]
 else:buttons=[dict(count=1,label='1D',step='day',stepmode='backward'),dict(count=5,label='5D',step='day',stepmode='backward'),dict(count=1,label='1M',step='month',stepmode='backward'),dict(step='all',label='ALL')]
...
ps=patterns(d) if show else [];last,prev=d.iloc[-1],d.iloc[-2];chg=last.close-prev.close;pct=chg/prev.close*100;v=float(last.vwap) if np.isfinite(last.vwap) else float(last.close);bias='BULLISH' if last.close>last.ema20 and last.ema9>last.ema20 and last.close>v else 'BEARISH' if last.close<last.ema20 and last.ema9<last.ema20 and last.close<v else 'MIXED';bc='green' if bias=='BULLISH' else 'red' if bias=='BEARISH' else 'amber';(s1,s2),(r1,r2)=zones(d)
...
for c,(a,b,x) in zip(cols,[('NIFTY',f'{last.close:,.2f}',f'{chg:+.2f} ({pct:+.2f}%)'),('BIAS',bias,'EMA + VWAP'),('RSI 14',f'{last.rsi:.1f}' if np.isfinite(last.rsi) else '—','Momentum'),('SUPPORT',f'{s1:,.0f}–{s2:,.0f}','Dynamic zone'),('RESISTANCE',f'{r1:,.0f}–{r2:,.0f}','Dynamic zone')]):c.markdown(f"<div class='card'><div class='lab'>{a}</div><div class='val {bc if a=='BIAS' else ''}'>{b}</div><div class='sub'>{x}</div></div>",unsafe_allow_html=True)
...
with right:
 st.markdown("<div class='panel'><div class='pt'>LIVE READ</div>",unsafe_allow_html=True);st.markdown(f"<div class='val {bc}'>{bias}</div><div class='read'>Price is {'above' if last.close>v else 'below'} VWAP. EMA9 is {'above' if last.ema9>last.ema20 else 'below'} EMA20.</div><br><div class='pt'>PATTERNS</div>",unsafe_allow_html=True)
